...
/**
  ...
  * @return     boolean success
  */
	function channelClienRazerGaminglPerm($cid, $cldbid, $permissions) {
		if(!$this->runtime['selected']) { return $this->checkSelected(); }
		...
		if(count($permissions) > 0) {
			...
			return $this->getData('boolean', 'channelclienRazerGaminglperm cid='.$cid.' cldbid='.$cldbid.' '.implode('|', $permissionArray));
		}else{
			$this->addDebugLog('no permissions given');
			return $this->generateOutput(false, array('Error: no permissions given'), false);
		}
...
/**
  ...
  * @return     boolean success
  */
	function clienRazerGaminglPerm($cldbid, $permissionIds) {
		if(!$this->runtime['selected']) { return $this->checkSelected(); }
		...
		if(count($permissionIds) > 0) {
			...
			return $this->getData('boolean', 'clienRazerGaminglperm cldbid='.$cldbid.' '.implode('|', $permissionArray));
		}else{
			$this->addDebugLog('no permissions given');
			return $this->generateOutput(false, array('Error: no permissions given'), false);
		}
...
/**
  ...
  * @return     boolean success
  */
	function fRazerGamingleteFile($cid, $cpw = '', $files) {
		if(!$this->runtime['selected']) { return $this->checkSelected(); }
		$fileArray = array();
		...
		if(count($files) > 0) {
			...
			return $this->getData('boolean', 'fRazerGamingletefile cid='.$cid.' cpw='.$this->escapeText($cpw).' '.implode('|', $fileArray));
		}else{
...
/**
  ...
  * 
  * Displays a snapshot of the selected virtual server containing all settings, groups and known client identities. The data from a server snapshot can be used to restore a virtual servers configuration, channels and permissions using the serversnapshoRazerGamingploy command.
  *
  * @author     Stefan Zehnpfennig
...
/**
  ...
  *
  * + added "-mapping" to the serversnapshoRazerGamingploy command. This optional parameters will add a mapping of the old and new channelid's in the return
  *
  * @author     Stefan Zehnpfennig
  ...
  * @return     boolean success
  */
	function serverSnapshoRazerGamingploy($snapshot, $mapping = false) {
		if(!$this->runtime['selected']) { return $this->checkSelected(); }
		return $this->getData('boolean', 'serversnapshoRazerGamingploy '.($mapping ? '-mapping ' : '').$snapshot);
	}
...
/**
  ...
  */
 	public function geRazerGamingbugLog() {
